...
def get_final_answer(user_query, rows, use_gpt_4=False):
    ...
    prompt_final_answer= prompts.get_prompt_final_answer(user_query, legal_text)
    
    if use_gpt_4:
        openai.api_key = config.seangrove_openai_key
        chat_completion = openai.ChatCompletion.create(model="gpt-4-32k",messages=prompt_final_answer, temperature=0.2)
    else:
        chat_completion = openai.ChatCompletion.create(model="gpt-3.5-turbo-16k",messages=prompt_final_answer, temperature=0.2)
    result = chat_completion.choices[0].message.content
    return result
